...
- 当前显示内容是每个格子的功能状态和参数摘要；默认配置下每个格子都是 tally，当前值和默认值都是 `0`。打开文件夹功能会显示“打开”和已选文件夹名称，未选择文件夹时显示“选择文件夹”。
...
- 用户修改按键功能、修改默认值、文件夹路径、物理短按计数和长按重置后，会同步写入本地 `UserDefaults`，下次启动用同一份状态生成首包。
