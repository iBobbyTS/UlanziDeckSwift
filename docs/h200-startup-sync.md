...
- 当前显示内容是每个格子的功能状态和参数摘要；默认配置下每个格子都是 tally，当前值和默认值都是 `0`。打开文件夹和打开文件功能会显示已选项目名称，未选择时分别显示“选择文件夹”和“选择文件”。打开文件会保存选择时读取到的系统文件图标两版按键背景：直接缩小到长边 `196px`，以及高斯模糊后缩小到长边 `196px`；参数面板的“高斯模糊”按钮只在已选择文件且图标正常获取后可用，按键图使用和文件夹相同的文字叠加排版，并受按钮背景明暗开关控制。旧配置没有图标时只显示名称。亮度是顶部全局 slider，不占用任何按键格子。
