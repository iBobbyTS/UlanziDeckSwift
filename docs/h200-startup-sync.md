...
- 启动时先通过 `UserDefaultsDeckConfigurationStore` 尝试恢复用户上次保存的页面树配置；没有保存内容、保存内容损坏或布局不匹配时，回退到 `DeckGridInteractionState(layout: .h200Prototype)` 的默认根页面配置。旧版 flat 14 格配置会迁移为 `root` 页面。
- 初始包总是从根页面生成。页面树会持久化每一页的 14 个格子和嵌套关系，但当前停留页面只属于运行时状态，不写入 `UserDefaults`；应用重启、重新发现设备或重新同步后都会回到根页面。
- 初始包使用恢复后的 `DeckGridInteractionState.displays(for: .h200Prototype)` 生成当前页 14 个格子的标题和副标题。
- 当前显示内容是每个格子的功能状态和参数摘要；默认配置下根页面每个格子都是 tally，当前值和默认值都是 `0`。新建页面文件夹时，子页面第 14 个宽槽位默认继承父页面当时的第 14 个宽槽位显示模式（功能、时钟或系统状态），但不继承父页面第 14 个宽槽位的功能配置；按键 13 默认放置返回键，返回键默认显示名称为空且不变暗。页面文件夹默认名为“功能夹”，功能列表和默认按键背景都使用系统 `folder` 符号；Finder 打开文件夹功能保持原来的“打开文件夹”文案和 `FolderBackground` 默认背景。每个按键配置都使用按键级 `DeckKeyVisualConfiguration` 保存显示名称、自定义背景图、高斯模糊开关和变暗开关；右侧参数面板把这些视觉控制统一放在“功能”下面。选择背景时，选择器附件下拉为“图像”会读取图像本身并生成最长边不超过 `512px` 的原图与高斯模糊版本，原图不足该尺寸时不放大；下拉为“所有文件”会读取所选文件的系统 icon 并生成同样两版。打开文件夹、SMB 和游戏默认背景在 app 内只打包最长边 `512px` 的 PNG 源图，不打包预生成缩略图；打开网页、页面文件夹和返回键默认背景运行时分别从系统 `globe`、`folder` 和 `arrow.uturn.left` 符号生成；填写网页地址后会异步获取网页标题和 icon，标题作为该功能的自动显示名，icon 作为该按键实例的默认背景；运行时按每个按键实例生成默认原图和高斯模糊背景数据，删除按钮功能后这些实例背景数据会随配置一起清空。清除自定义背景后会回到该实例自己的默认背景来源：打开文件优先使用所选文件 icon，打开网页使用已获取的网页 icon、未获取前使用系统 `globe` 符号，打开文件夹使用 `FolderBackground` 源图，页面文件夹使用系统 `folder` 符号，返回键使用系统 `arrow.uturn.left` 符号，SMB 使用 `SMBServerBackground` 源图，游戏状态使用对应游戏源图，其余功能使用黑底。参数面板的“高斯模糊”和“变暗”都是按键级设置，每个实例可以独立切换。顶部亮度 slider 只控制设备亮度，不占用任何按键格子。
...
- 进入页面文件夹或返回上一级页面会切换当前运行时页面，并重新发送一份完整 14 格显示包；隐藏页面上的定时刷新、长按和异步请求会在导航前取消，导航后只启动当前可见页面需要的运行时任务。
...
- 用户修改按键功能、修改默认值、文件夹路径、文件路径、页面文件夹结构、亮度百分比、物理短按计数和长按重置后，会同步写入本地 `UserDefaults`，下次启动用同一份页面树的根页面生成首包。
...
- 页面文件夹导航和返回导航不使用局部更新，而是发送完整 `0x0001` 14 格包，确保设备端直接切到目标页面的完整画面。
