...
- 当前显示内容是每个格子的功能状态和参数摘要；默认配置下根页面每个格子都是 tally，当前值和默认值都是 `0`。每个按键配置都使用按键级 `DeckKeyVisualConfiguration` 保存显示名称、自定义背景图、高斯模糊版本和变暗开关；右侧参数面板把这些视觉控制统一放在“功能”下面。选择背景时，选择器附件下拉为“图像”会读取图像本身并生成长边 `196px` 的原图与高斯模糊版本；下拉为“所有文件”会读取所选文件的系统 icon 并生成同样两版。清除自定义背景后会回到该功能自己的默认背景来源：打开文件优先使用所选文件 icon，打开文件夹、SMB 和页面文件夹分别使用内置 `FolderBackground`、`SMBServerBackground` 和 `PageFolderBackground`，游戏状态使用对应游戏背景，其余功能使用黑底。参数面板的“高斯模糊”按钮只在当前自定义背景或默认文件 icon 存在模糊版本时可用；“变暗”是按键级设置，不再有顶部全局背景变暗开关。返回键默认使用黑底居中“返回”文字。顶部亮度 slider 只控制设备亮度，不占用任何按键格子。
