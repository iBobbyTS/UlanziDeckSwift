...
- 当前显示内容是每个格子的功能状态和参数摘要；默认配置下根页面每个格子都是 tally，当前值和默认值都是 `0`。新建页面文件夹时，子页面第 14 个宽槽位默认继承父页面当时的第 14 个宽槽位显示模式（功能、时钟或系统状态），但不继承父页面第 14 个宽槽位的功能配置。页面文件夹默认名为“功能夹”，功能列表和默认按键背景都使用系统 `folder` 符号；Finder 打开文件夹功能保持原来的“打开文件夹”文案和 `FolderBackground` 默认背景。每个按键配置都使用按键级 `DeckKeyVisualConfiguration` 保存显示名称、自定义背景图、高斯模糊开关和变暗开关；右侧参数面板把这些视觉控制统一放在“功能”下面。选择背景时，选择器附件下拉为“图像”会读取图像本身并生成最长边不超过 `512px` 的原图与高斯模糊版本，原图不足该尺寸时不放大；下拉为“所有文件”会读取所选文件的系统 icon 并生成同样两版。打开文件夹、SMB 和游戏默认背景在 app 内只打包最长边 `512px` 的 PNG 源图，不打包预生成缩略图；页面文件夹默认背景运行时从系统 `folder` 符号生成；运行时按每个按键实例生成默认原图和高斯模糊背景数据，删除按钮功能后这些实例背景数据会随配置一起清空。清除自定义背景后会回到该实例自己的默认背景来源：打开文件优先使用所选文件 icon，打开文件夹使用 `FolderBackground` 源图，页面文件夹使用系统 `folder` 符号，SMB 使用 `SMBServerBackground` 源图，游戏状态使用对应游戏源图，其余功能使用黑底。参数面板的“高斯模糊”和“变暗”都是按键级设置，每个实例可以独立切换。返回键默认使用黑底居中“返回”文字。顶部亮度 slider 只控制设备亮度，不占用任何按键格子。
