...
struct UlanziDeckSwiftTests {
    ...
    @Test func zcodePreRenderedBackgroundKeepsOriginalBrightnessForDimmingToggle() throws {
        ...
        #expect(brightColor.redComponent > 0.9)
        #expect(abs(brightColor.redComponent - brightColor.greenComponent) < 0.01)
        #expect(abs(brightColor.greenComponent - brightColor.blueComponent) < 0.01)
    }
    ...
    @Test func buttonDimmingUsesSharedDefaultAndZcodeOverride() throws {
        ...
        let didAssignCodexUsage = state.assign(.codexUsage, to: 2)
        #expect(didAssignCodexUsage)
        #expect(state.configuration(for: 2)?.visual.dimmingPercent == 38)
        let didAssignZcodeUsage = state.assign(.zcodeUsage, to: 3)
        #expect(didAssignZcodeUsage)
    ...
    @Test func userDefaultsStoreMigratesBalanceOnlyLegacyPlaintextCredentialForDifferentLayout() throws {
        let suiteName = "UlanziDeckSwiftTests.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
